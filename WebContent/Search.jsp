...
<body>
	...
	<%
		...
		String user = null;
		Vector<PersonBean> vpbSearch = new Vector<PersonBean>();
		vpbSearch = null;
		...
		if (session.getAttribute("userLogin") == null) {
			...
			vpbSearch = (Vector<PersonBean>) session
					.getAttribute("searchedResult");
		...
		<div>
			<section class="upstatus">
				...
				<table class="hovertable">
					...
					<%
						for (int i = 0; i < vpbSearch.size(); i++) {
					%>
					<tr onmouseover="this.style.backgroundColor='#ffffff';"
						onmouseout="this.style.backgroundColor='#c0deed';">
						<td><%=vpbSearch.elementAt(i).getFullName()%></td>
						<td><a class="tweetUserProfile"
							href="ControllerServlet?userid=<%=vpbSearch.elementAt(i).getUserName()%>"><%=vpbSearch.elementAt(i).getUserName()%></a></td>
						<td><%=vpbSearch.elementAt(i).getEmail()%></td>
					</tr>

					<%
						}
							session.removeAttribute("searchedResult");
					%>
				</table>
				...
				<p>
					<font size="5" color="red">Sorry no results found!</font>
				</p>
